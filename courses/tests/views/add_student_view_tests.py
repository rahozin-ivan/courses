...
class AddStudentViewTests(APITestCase):
    def setUp(self) -> None:
        ...
        self.url = reverse('course_add_student', args=[self.course.pk])

    def test_add_student_view_not_working_without_login(self):
        self.client.logout()
        res = self.client.post(self.url)
        assert status.HTTP_401_UNAUTHORIZED == res.status_code

    def test_add_student_view_not_working_for_not_course_users(self):
        self.client.force_authenticate(mixer.blend(get_user_model()))
        res = self.client.post(self.url)
        assert status.HTTP_403_FORBIDDEN == res.status_code

    def test_add_student_view_not_working_for_course_students(self):
        self.client.force_authenticate(self.student)
        res = self.client.post(self.url)
        assert status.HTTP_403_FORBIDDEN == res.status_code

    def test_add_student_view_working_correctly(self):
    ...
    def test_add_student_view_not_working_when_student_does_not_exists(self):
        email = 'notemail'
        data = {
            'email': email,
        }
    ...
    def test_add_student_view_not_working_if_user_already_course_teacher(self):
        ...
        res = self.client.post(self.url, data)
        assert status.HTTP_404_NOT_FOUND == res.status_code
        assert f'Пользователь с таким {email=} является преподавателем' == res.data
